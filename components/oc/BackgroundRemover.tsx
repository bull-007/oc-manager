...
export default function BackgroundRemover({ imageUrl, onCutoutComplete, onClose }: Props) {
  ...
  const initCanvas = (dataUrl: string) => {
    ...
    img.onload = () => {
      ...
      origDataRef.current = new ImageData(new Uint8ClampedArray(imgDataRef.current.data), canvas.width, canvas.height);
    };
    img.src = dataUrl;
  };
  ...
  const drawAt = useCallback((cx: number, cy: number) => {
    ...
    const w = imgData.width, h = imgData.height, r = brushSize;
    for (let dy = -r; dy <= r; dy++) {
      for (let dx = -r; dx <= r; dx++) {
        ...
        const i = (py * w + px) * 4;
        imgData.data[i + 3] = tool === "erase" ? 0 : origData.data[i + 3];
      }
    }
  ...
  return (
    <div className="space-y-3" onClick={(e) => e.stopPropagation()}>
      <div className="flex gap-4">
        ...
        <div className="flex-1">
          <p className="text-xs text-warm-muted mb-1">结果预览 · 可直接涂抹修图</p>
      ...
      <div>
        ...
        <div className="flex justify-between text-[10px] text-warm-muted"><span>更透明</span><span>更保留</span></div>
      </div>

      {/* Manual tools */}
      <div className="flex items-center gap-3 flex-wrap">
        <span className="text-xs text-warm-muted font-medium">手动修图：</span>
      ...
      <div className="flex justify-end gap-3">
        <button onClick={onClose} disabled={processing}
          className="px-4 py-2 text-sm text-warm-muted hover:text-warm-brown border border-warm-border rounded-lg disabled:opacity-50">取消</button>
