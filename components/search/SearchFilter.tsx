    ...
    <div className="space-y-3">
      <div className="flex gap-2">
        <div className="relative flex-1">
          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-warm-muted/50">🔍</span>
          <input type="text" value={search} onChange={(e) => onSearchChange(e.target.value)}
            placeholder="搜索 OC 名称、种族、职业..."
            className="w-full pl-10 pr-4 py-2.5 border border-warm-border rounded-lg bg-warm-paper text-warm-brown placeholder-warm-muted focus:outline-none focus:ring-2 focus:ring-amber-400 text-sm" />
        </div>
        <button onClick={() => setShowFilters(!showFilters)}
          className={`px-4 py-2.5 border transition-colors text-sm ${showFilters || hasFilters ? "bg-amber-100/60 border-amber-300 text-amber-700" : "border-warm-border text-warm-muted hover:bg-warm-paper"}`}
      ...
      {showFilters && (
        <div className="flex flex-wrap gap-4 p-4 bg-warm-paper border border-warm-border rounded-lg animate-slide-up">
          ...
          <div className="flex-1 min-w-[150px]">
            ...
            <input type="text" value={tag} onChange={(e) => onTagChange(e.target.value)}
              placeholder="输入标签名..." className="w-full px-3 py-2 border border-warm-border rounded-lg bg-warm-cream text-sm" />
          </div>
