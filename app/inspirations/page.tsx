...
export default function InspirationsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success("灵感已记录 ✧");
        setContent(""); setTags([]); setRelatedOcId(""); setShowForm(false);
        mutate();
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-slide-up">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-hand text-warm-brown">灵感速记</h1>
          <p className="text-warm-muted text-sm mt-1">快速记录创作灵感，随时捕捉闪现的想法</p>
        </div>
      ...
      {showForm && (
        <Card padding="md" className="animate-slide-up">
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-warm-brown mb-1">灵感内容 *</label>
            ...
            <div>
              <label className="block text-sm font-medium text-warm-brown mb-1">标签</label>
              <TagInput tags={tags} onChange={setTags} placeholder="添加标签..." />
            </div>
            <div>
              <label className="block text-sm font-medium text-warm-brown mb-1">关联 OC（可选）</label>
            ...
            <div className="flex justify-end">
              <Button onClick={handleSave} disabled={saving}>{saving ? "保存中..." : "记录灵感 ✧"}</Button>
            </div>
          </div>
        </Card>
      )}
      ...
      {!isLoading && inspirations.length === 0 && !showForm && (
        <div className="bg-warm-paper border border-dashed border-warm-border rounded-xl p-12 text-center">
          <p className="text-warm-muted mb-3">还没有记录任何灵感</p>
          <p className="text-xs text-warm-muted">点击上方按钮，开始记录你的第一个灵感</p>
        </div>
      )}
      ...
      <div className="columns-1 sm:columns-2 lg:columns-3 gap-5 space-y-5">
        {inspirations.map((insp: any) => {
          ...
          return (
            <div key={insp.id}
              className="break-inside-avoid bg-warm-paper border border-warm-border p-4 hover:shadow-md transition-shadow group"
              style={{
                borderRadius: "18px 5px 18px 5px / 15px 4px 15px 4px",
                transform: `rotate(${(insp.id.charCodeAt(0) % 5 - 2) * 0.4}deg)`,
              }}>
              <p className="text-sm text-warm-brown whitespace-pre-wrap leading-relaxed mb-3">{insp.content}</p>
              {inspTags.length > 0 && (
                <div className="flex flex-wrap gap-1 mb-2">
                  {inspTags.map((tag: string, i: number) => <span key={i} className="tag-pill">{tag}</span>)}
                </div>
              )}
              {insp.relatedOc && <p className="text-xs text-warm-muted mb-2">◆ {insp.relatedOc.name}</p>}
              <div className="flex items-center justify-between">
                <span className="text-xs text-warm-muted">{formatDate(insp.createdAt)}</span>
