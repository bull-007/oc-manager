...
export default async function DashboardPage() {
  ...
  const stats = [
    { label: "OC 角色", value: ocCount, href: "/ocs" },
    { label: "世界观", value: worldCount, href: "/worlds" },
    { label: "关系", value: relationCount, href: "/relations" },
    { label: "灵感", value: inspirationCount, href: "/inspirations" },
  ];
    ...
    <div className="max-w-3xl mx-auto space-y-10 animate-slide-up">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {stats.map((s) => (
          <Link key={s.label} href={s.href}
            className="bg-stone-card border border-stone-border p-5 hover:shadow-md hover:-translate-y-0.5 hover:border-sage transition-all duration-200"
            style={{ borderRadius: "10px" }}>
            <div className="text-3xl font-serif font-medium text-stone-text mb-1">{s.value}</div>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <Link href="/worlds/new"
          className="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-medium text-stone-text border border-stone-border hover:bg-stone-hover transition-colors"
          style={{ borderRadius: "8px" }}>+ 新建世界观</Link>
        <Link href="/inspirations"
          className="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-medium text-stone-text border border-stone-border hover:bg-stone-hover transition-colors"
          style={{ borderRadius: "8px" }}>记录灵感</Link>
      </div>

      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-serif font-medium text-stone-text">最近编辑的 OC</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {recentOCs.map((oc) => (
              <Link key={oc.id} href={`/ocs/${oc.id}/panel`}
                className="bg-stone-card border border-stone-border flex items-center gap-4 p-4 hover:shadow-md hover:-translate-y-0.5 hover:border-sage transition-all duration-200"
                style={{ borderRadius: "10px" }}>
                ...
                <div className="min-w-0">
                  ...
                  {oc.status === "draft" && <span className="inline-block text-xs mt-1 px-1.5 py-0.5 rounded bg-stone-hover text-stone-muted">草稿</span>}
                </div>
              </Link>
            ))}
          </div>
